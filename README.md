...
`wpcli` runs a WP-CLI command against the configured site over SSH, for work no structured command covers. Put the arguments after `--` when they include options, for example `wp-agent --json wpcli -- post list --post_type=page --format=json`. A command is read-only when known command nouns are followed by a read verb, such as `post list`, `option get`, `post meta get`, or `plugin is-active`; it runs directly and is recorded in the journal. Anything else, including `eval`, `db query`, and `search-replace`, is a `wpcli.run` raw mutation: the policy always requires approval, and the proposed action carries the full argument list, so an approval covers only that exact command. `--exec` and `--require` make any command a mutation. `--ssh`, `--http`, `--path`, `--prompt`, `shell`, and `db cli` are refused. JSON output includes `stdout` and, when it parses, `data`. WP-CLI mutations take no snapshot; export the state you may need before approving one.
