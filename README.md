...
`wpcli` runs a WP-CLI command against the configured site over SSH, for work no structured command covers. Put the arguments after `--` when they include options, for example `wp-agent --json wpcli -- post list --post_type=page --format=json`. The runner passes the configured site URL to WP-CLI. A command is read-only only when its command path is on the explicit allowlist in `src/wpcli.ts`, such as `post list`, `option get`, `post meta get`, or `plugin is-active`; it runs directly and is recorded in the journal. Anything else, including unknown plugin commands, `eval`, `db query`, and `search-replace`, is a `wpcli.run` raw mutation: the policy always requires approval, and the proposed action carries the full argument list, so an approval covers only that exact command. `--exec` and `--require` make any command a mutation. `--ssh`, `--http`, `--path`, `--url`, `--config`, `--prompt`, `shell`, and `db cli` are refused. JSON output includes `stdout` and, when it parses, `data`. WP-CLI mutations take no snapshot; export the state you may need before approving one.
