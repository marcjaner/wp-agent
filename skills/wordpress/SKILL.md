...
- Check `policy explain` for consequential actions. Follow the policy decision and the user's scope. Publishing, deleting preexisting content, changing published production pages, and production site-wide or plugin/theme changes can require explicit approval. A `--yes` flag does not grant that approval. `policy explain` only previews a decision; it does not issue an approval request.
- Run mutations with `--json`. If a command returns `policy_decision` and `error.policy.decision` is `require_approval`, show the user the exact `error.action` and impact, then wait for explicit approval in the conversation. Retry the **same command** with global `--approval-request <error.approvalRequestId> --approval-note "<brief reference to the user's approval>"`. Use neither flag without actual approval. The request is one-use and tied to the session, site, action, payload, and observed content state. If a retry does not match, get a new request and ask again. wp-agent records the agent's attestation; it cannot verify chat consent itself. Interactive direct CLI use still supports typing `approve`.
- Use raw REST or WP-CLI mutation paths only when a structured operation cannot do the requested work, and keep them inside wp-agent's policy boundary. For WP-CLI, use `wp-agent --json wpcli -- <args>`: reads run directly, and mutations return a `policy_decision` that follows the same approval retry. Show the user the full `error.action.input.args` when asking for approval. SSH/WP-CLI, the bridge plugin, and Jev are optional; discover capabilities before relying on them. Theme installation and activation currently require SSH/WP-CLI. Never put credentials in command arguments, URLs, logs, or committed files.
