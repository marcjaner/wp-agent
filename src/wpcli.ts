...
export async function remoteWp(args: string[]): Promise<string> {
  ...
  if (!read && !insidePolicyExecution()) {
    return executeMutation({ tool: 'wp-cli.raw', category: 'raw', mutation: true, target: { type: 'wp-cli', id: args[0] || 'unknown' }, reversible: 'partial', input: { command: args.slice(0, 2).map((value, index) => index === 1 && args[0] === 'eval' ? '[code]' : value) } }, () => remoteWp(args), { site: siteUrl(), interactive: !process.argv.includes('--json') });
  }
