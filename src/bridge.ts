...
import { executeMutation, hashPayload, insidePolicyExecution, savePolicySnapshot } from './policy.js';
...
export class BridgeClient {
  ...
  async writeSettings(settings: ThemeSettings['settings']): Promise<ThemeSettings> {
    ...
    if (!insidePolicyExecution()) {
      ...
      return executeMutation({ tool: 'theme.settings.set', category: 'site_config', mutation: true, target: { type: 'theme', id: before.stylesheet }, intent: { changes: Object.keys(settings) }, reversible: 'reversible', input: { fields: Object.keys(settings), settingsHash: hashPayload(settings), beforeHash: hashPayload(before.settings) } }, () => this.writeSettings(settings), { site: this.wp.url, snapshot: () => savePolicySnapshot('theme-settings', before) });
    }
  ...
  async writeCss(css: string, expectedHash: string): Promise<CssState> {
    ...
    if (!insidePolicyExecution()) {
      ...
      return executeMutation({ tool: 'custom-css.set', category: 'site_config', mutation: true, target: { type: 'custom-css', id: before.stylesheet }, intent: { changes: ['css'] }, reversible: 'reversible', input: { stylesheet: before.stylesheet, cssLength: css.length, cssHash: hashPayload(css), expectedHash } }, () => this.writeCss(css, expectedHash), { site: this.wp.url, snapshot: () => savePolicySnapshot('custom-css', before) });
    }
