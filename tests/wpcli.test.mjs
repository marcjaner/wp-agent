...
test('WP-CLI reads are limited to known nouns followed by a read verb', () => {
  ...
  for (const args of [['eval', 'echo 1;'], ['option', 'update', 'list', 'x'], ['post', 'update', '5', 'get'], ['search-replace', 'a', 'b'], ['db', 'query', 'SELECT 1'], ['list'], ['option', 'get', 'blogname', '--exec=wp_delete_post(1);'], ['cron', 'event', 'run', '--all']])
...
test('wpcli runs reads directly and journals them', async () => {
  ...
  assert.match(ssh.calls()[0], /'wp' '--path=\/srv\/wp' 'post' 'list' '--format=json'/);
...
test('wpcli refuses retargeting flags without contacting the server', async () => {
  ...
  startSession('Inspect posts.', 'production', site, ssh.file);
  let refused;
  try { await ssh.run(['wpcli', 'post', 'list', '--ssh=other.host']); } catch (error) { refused = JSON.parse(error.stderr); }
  assert.match(refused.error.message, /--ssh is not allowed/);
  assert.equal(ssh.calls().length, 0);
});
